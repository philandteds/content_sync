			...
			<table class="list" cellspacing="0" cellpadding="0">
				...
				<tbody>
					...
					<tr class="{$style}">
						...
						<td>{if $log.object}<a href="{concat( '/content/versionview/', $log.object_id, '/', $log.object_version, '/', $log.version.initial_language.locale )|ezurl( 'no' )}" target="_blank">{$log.object_version} ({$log.version.initial_language.locale})</a>{else}{$log.object_version} ({$log.version.initial_language.locale}){/if}</td>
